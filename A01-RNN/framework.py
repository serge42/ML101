...
class Sigmoid:
    ...
    def backward(self, error):
        ...
        # we already computed sig(x), use d/dx(sig(x)) = sig(x)(1-sig(x))
        d_x = sigmoid_x * (1 - sigmoid_x) * error # HOW COULD FORGET THE ERROR???
...
class Linear:
    ...
    def backward(self, error):
        ...
        W = self.var['W']
        b = self.var['b']
        dW = np.dot(x.transpose(), error)
        db = np.sum(error, axis=0, keepdims=False)
        d_inputs = np.dot(error, W.transpose())
        # TODO update weights: I probabliy should not do this here since I don't have access to learning_rate
        # but I don't know where else it could be done
        # W -= 0.1 * dW
        # b -= 0.1 * db
        ## End
...
class MSE:
    def forward(self, prediction, target):
        ...
        err_square = (T - Y)**2
        meanError = np.sum(err_square, keepdims=True) / (2 * n) # SAME ERROR THAN IN EX.1!
        self.saved_variables = {
            "Y": Y,
            "T": T,
            "n": n
...
# model = create_network(), loss = MSE(), learning_rate -> scalar, inputs,targets \in R^Nx2
def train_one_step(model, loss, learning_rate, inputs, targets):
    ## Implement
    # Forward pass
    net = model.forward(inputs)
    error = loss.forward(net, targets)
    # Backward pass
    dE = loss.backward()
    variable_grads, smt = model.backward(learning_rate * dE)
    ## End
    return error
...
def gradient_check():
    ...
    for key, value in NN.var.items():
        ...
        for index in range(variable.shape[0]):
            ...
            ## Implement
            # TODO: I Honnestly have no idea what I'm doing here... 
            # How can I compute a gradient using only one element of Wi??
            v = variable[index]
            x = X.reshape(-1)
            analytic_grad = ((X-eps)*variable[index] + (X+eps)*variable[index]) / (2*eps)
